...
## How it Works (Video)
To understand what Analyst Agent does and how it works watch this video.
...
## Why Analyst Agent was built
Analyst Agent is a tool we built so that we (at craft.do) can work effectively with agents. It enables intuitive multitasking, no-fluff connection to any API or Service, sharing sessions, and a more document (vs code) centric workflow - in a beautiful and fluid UI.
...
Analyst Agent is open source under the Apache 2.0 license - so you are free to remix, change anything. And that's actually possible. We ourselves are building Analyst Agent with Analyst Agent only - no code editors - so really, any customisation is just a prompt away.

We built Analyst Agent because we wanted a better, more opinionated (and preferably non-CLI way) of working with the most powerful agents in the world. We'll continue to improve it, based on our experiences and intuition.
...
[Check out how I just connected to Slack →](https://github.com/yifuwang0421/AnalystAgent/s/DRNQEiy8w2e1v5LPgKl8b)
...
Analyst Agent connects to anything. We have it hooked up to a direct Postgres DB behind a jumpbox. Skills + Sources = magic.
...
[Here I imported all my skills in one go →](https://github.com/yifuwang0421/AnalystAgent/s/gWCFqwhObFWaNJIEJmd6j)
...
**macOS / Linux:**
```bash
curl -fsSL https://github.com/yifuwang0421/AnalystAgent/install-app.sh | bash
```

**Windows (PowerShell):**
```powershell
irm https://github.com/yifuwang0421/AnalystAgent/install-app.ps1 | iex
```
...
Analyst Agent can run as a headless server on a remote machine (e.g., a Linux VPS), with the desktop app connecting as a thin client. This lets you keep long-running sessions alive, access them from multiple machines, and run compute-heavy tasks on a powerful server.
...
docker run -d \
  ...
  -v craft-data:/root/.analyst-agent \
  craft-agents-server
```
...
A terminal client that connects to a running Analyst Agent server over WebSocket (`ws://` or `wss://`). Use it for scripting, CI/CD pipelines, server validation, or when you prefer the command line.
...
4. Name: e.g., "Analyst Agent Desktop"
5. Click **Create**
6. Note the **Client ID** and **Client Secret**

#### 5. Configure in Analyst Agent
...
Analyst Agent supports multiple ways to connect to LLM providers:
...
Analyst Agent uses two agent backends:
...
Configuration is stored at `~/.analyst-agent/`:
...
Or configure manually in `~/.analyst-agent/workspaces/{id}/automations.json`:
...
See the [Automations documentation](https://github.com/yifuwang0421/AnalystAgent/docs/automations/overview) for the full reference.
...
External apps can navigate using `analystagent://` URLs:

```
analystagent://allSessions                      # All sessions view
analystagent://allSessions/session/session123   # Specific session
analystagent://settings                         # Settings
analystagent://sources/source/github            # Source info
analystagent://action/new-chat                  # Create new session
```
...
"Craft" and "Analyst Agent" are trademarks of Craft Docs Ltd. See [TRADEMARK.md](TRADEMARK.md) for usage guidelines.
