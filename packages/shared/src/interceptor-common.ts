...
export const CONFIG_FILE = join(homedir(), '.analyst-agent', 'config.json');
...
export const LOG_DIR = join(homedir(), '.analyst-agent', 'logs');
...
function getErrorFilePath(): string {
  ...
  return join(homedir(), '.analyst-agent', 'api-error.json');
}
