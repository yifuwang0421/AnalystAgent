...
import { ensureFinanceWorkspaceConfig, getDefaultFinanceWorkspaceConfig } from './finance.ts';
...
export function loadWorkspaceConfig(rootPath: string): WorkspaceConfig | null {
  ...
  try {
    ...
    if (config.finance?.researchDirectory) {
      config.finance.researchDirectory = expandPath(config.finance.researchDirectory);
    }
...
export function saveWorkspaceConfig(rootPath: string, config: WorkspaceConfig): void {
  ...
  if (storageConfig.finance?.researchDirectory) {
    storageConfig.finance = {
      ...storageConfig.finance,
      researchDirectory: toPortablePath(storageConfig.finance.researchDirectory),
    };
  }
...
export function loadWorkspace(rootPath: string): LoadedWorkspace | null {
  ...
  if (ensureFinanceWorkspaceConfig(rootPath, config)) {
...
export function createWorkspaceAtPath(
  ...
  const config: WorkspaceConfig = {
    ...
    finance: getDefaultFinanceWorkspaceConfig(rootPath),
    createdAt: now,
    updatedAt: now,
  };
  ...
  ensureFinanceWorkspaceConfig(rootPath, config);
