 ...
 *
 * File locations:
 * - Workspace: ~/.analyst-agent/workspaces/{slug}/permissions.json
 * - Per-source: ~/.analyst-agent/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.analyst-agent/permissions/
 * Reads env var dynamically so tests can override via CRAFT_CONFIG_DIR.
 */
export function getAppPermissionsDir(): string {
  const configDir = process.env.CRAFT_CONFIG_DIR || join(homedir(), '.analyst-agent');
...
/**
 * Load default permissions from ~/.analyst-agent/permissions/default.json
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.analyst-agent/permissions/default.json)
   ...
   * These come from ~/.analyst-agent/permissions/default.json
   */
