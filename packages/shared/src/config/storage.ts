...
const LEGACY_ANALYST_AGENT_TOOL_DISPLAY_NAME = ['Craft', 'Agent'].join(' ');
const LEGACY_ANALYST_AGENT_TOOL_ICON = ['craft-agent', 'svg'].join('.');
...
export function ensureToolIcons(): void {
  ...
  migrateAnalystAgentToolIcon(toolIconsDir, bundledToolIconsDir);
}

function migrateAnalystAgentToolIcon(toolIconsDir: string, bundledToolIconsDir: string): void {
  try {
    const bundledIconPath = join(bundledToolIconsDir, 'analyst-agent.png');
    const targetIconPath = join(toolIconsDir, 'analyst-agent.png');
    if (!existsSync(targetIconPath) && existsSync(bundledIconPath)) {
      copyFileSync(bundledIconPath, targetIconPath);
    }
    ...
    const config = JSON.parse(raw) as {
      tools?: Array<{ id?: string; displayName?: string; icon?: string; commands?: string[] }>;
    };

    const tool = config.tools?.find(entry =>
      entry.id === 'craft-agent' ||
      entry.displayName === LEGACY_ANALYST_AGENT_TOOL_DISPLAY_NAME ||
      entry.icon === LEGACY_ANALYST_AGENT_TOOL_ICON
    );
    if (!tool) return;

    if (tool.displayName === 'Analyst Agent' && tool.icon === 'analyst-agent.png') return;

    tool.displayName = 'Analyst Agent';
    tool.icon = 'analyst-agent.png';
