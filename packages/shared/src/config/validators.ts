  ...
  try {
    ...
    return {
      ...
      errors: [{
        ...
        severity: 'error',
        suggestion: 'See ~/.analyst-agent/docs/skills.md for SKILL.md format reference',
      }],
      warnings: [],
    };
  }
...
/**
 * Zod schema for app-level theme override files (~/.analyst-agent/theme.json).
        ...
        if (tool.icon) {
          ...
          if (!existsSync(iconPath)) {
            result.warnings.push({
              ...
              severity: 'warning',
              suggestion: `Place '${tool.icon}' in ~/.analyst-agent/tool-icons/`,
            });
          }
        }
      }
 ...
 * Checks paths relative to CONFIG_DIR (~/.analyst-agent/).
 * Returns null if the path is not a recognized app-level config file.
 *
 * Matches patterns:
 * - ~/.analyst-agent/tool-icons/tool-icons.json → tool icon mappings
 */
