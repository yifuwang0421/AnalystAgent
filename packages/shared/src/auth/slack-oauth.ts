...
export function prepareSlackOAuth(options: PrepareSlackOAuthOptions): PreparedOAuthFlow {
  ...
  const redirectUri = options.callbackUrl
    ?? `https://github.com/yifuwang0421/AnalystAgent/auth/slack/callback?port=${options.callbackPort}`;
...
export async function startSlackOAuth(options: SlackOAuthOptions = {}): Promise<SlackOAuthResult> {
  try {
    ...
    // The relay redirects: https://github.com/yifuwang0421/AnalystAgent/auth/slack/callback → http://localhost:{port}/callback
    const redirectUri = `https://github.com/yifuwang0421/AnalystAgent/auth/slack/callback?port=${port}`;
