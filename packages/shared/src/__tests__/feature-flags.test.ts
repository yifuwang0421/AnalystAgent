...
import { isDevRuntime, isDeveloperFeedbackEnabled, isAnalystAgentCliEnabled, isEmbeddedServerEnabled } from '../feature-flags.ts';
...
describe('feature-flags runtime helpers', () => {
  ...
  it('isAnalystAgentCliEnabled defaults to false when no override is set', () => {
    delete process.env.CRAFT_FEATURE_CRAFT_AGENTS_CLI;

    expect(isAnalystAgentCliEnabled()).toBe(false);
  });

  it('isAnalystAgentCliEnabled honors explicit override true', () => {
    process.env.CRAFT_FEATURE_CRAFT_AGENTS_CLI = '1';

    expect(isAnalystAgentCliEnabled()).toBe(true);
  });

  it('isAnalystAgentCliEnabled honors explicit override false', () => {
