...
export const UpdatePreferencesSchema = z.object({
  ...
  includeCoAuthoredBy: z.boolean().optional().describe("Whether to include 'Co-Authored-By: Analyst Agent' trailer on git commits. Defaults to true."),
});
...
export const KnowledgeSearchSchema = z.object({
  query: z.string().describe('Search query for the workspace research knowledge base'),
  maxResults: z.number().optional().describe('Maximum results to return (1-20, default 8)'),
  includeDirectories: z.array(z.string()).optional().describe('Workspace-relative directories to search. Defaults to knowledge, companies, industries, reports.'),
});
...
export const FinanceMarketDataSchema = z.object({
  ...
  ]).describe('Kind of finance data request'),
  query: z.string().optional().describe('Company/fund/macro search query or indicator name'),
  symbol: z.string().optional().describe('Ticker, fund code, or instrument identifier'),
  assetType: z.enum(['stock', 'fund', 'macro', 'auto']).optional().describe('Asset class hint. Defaults to auto.'),
  provider: z.enum(['auto', 'ifind', 'yfinance', 'edgartools', 'akshare', 'baostock', 'python']).optional()
    .describe('Data provider. Auto routes by market scope and request type. Python-backed providers are read-only optional local runtimes.'),
  marketScope: z.enum(['cn-hk', 'us', 'global']).optional().describe('Market scope hint. Defaults to workspace finance config.'),
  startDate: z.string().optional().describe('Optional start date for historical/event-window requests, YYYY-MM-DD preferred.'),
  endDate: z.string().optional().describe('Optional end date for historical/event-window requests, YYYY-MM-DD preferred.'),
  period: z.string().optional().describe('Optional provider period, e.g. 1mo, 6mo, 1y.'),
  statementType: z.enum(['income', 'balance', 'cashflow', 'all']).optional().describe('Financial statement type. Defaults to all.'),
});
...
export const ResearchWorkflowSchema = z.object({
  taskType: z.enum(['auto', 'company_deep_research', 'earnings_review', 'event_impact', 'industry_scan']).optional()
    .describe('Research workflow type. Auto infers from target text.'),
  target: z.string().optional().describe('Company, ticker, industry, event, filing, or research object. Missing target returns HITL clarification.'),
  marketScope: z.enum(['cn-hk', 'us', 'global']).optional().describe('Market scope. Defaults to workspace finance config.'),
  asOfDate: z.string().optional().describe('Research cutoff/event date. Event-impact workflows should provide this.'),
  depth: z.enum(['quick', 'standard', 'deep']).optional().describe('Workflow depth. Defaults to standard.'),
  outputLanguage: z.enum(['zh-Hans', 'en']).optional().describe('Preferred output language. Defaults to Simplified Chinese.'),
  writeReport: z.boolean().optional().describe('Whether the workflow intends to write a report file inside the finance research directory.'),
});
...
- Do NOT include any text or tool calls after SubmitPlan - they will not be executed`,
  ...
  config_validate: `Validate Analyst Agent configuration files.
...
Templates use Mustache syntax — the tool handles rendering and writes the output HTML to the session data folder.`,
  ...
  knowledge_search: `Search the current workspace's finance research knowledge base.

Use this before drafting company research, earnings reviews, industry notes, or any answer that should use the workspace's stored research.

Searches workspace folders such as \`knowledge/\`, \`companies/\`, \`industries/\`, and \`reports/\`. Results include absolute file paths, title, snippet, updated timestamp, and whether the match came from content or filename. Binary files can match by filename and should be inspected with document tools when needed.`,

  finance_market_data: `Query configured finance market data through the generic provider interface.

Provider routing is read-only and research-only. Auto mode uses iFinD first for A-share/HK workspaces and optional Python-backed yfinance/edgartools for US public data. Supported request types include instrument search, quotes, historical prices, financial summaries/statements, valuation metrics, news/announcements, macro data, and technical indicators. The tool returns provider availability, raw payload, normalized payload, data timestamp, source citation, and warnings.

If the provider is unavailable or returns unstable text/Markdown payloads, do not stop the research. Continue with knowledge-base and user-provided files, and disclose the data-source limitation in the final answer.`,

  research_workflow: `Build a structured finance research workflow before complex analysis.

Use this for company deep research, earnings reviews, event-impact analysis, and industry scans. The workflow follows: Super Triage -> Research Planner -> Evidence Gathering -> Bull/Bear Review -> Risk/Verifier -> Research Manager Synthesis.

If required inputs are missing, the tool returns HITL clarification fields instead of guessing. It also returns recommended read-only tools, sub-agent role prompts for spawn_session, an evidence-ledger template, and an optional report write path constrained to the finance research directory.

Do not use this tool for trading execution. It is for research planning and evidence discipline only.`,
...
Only use 'attachments' for existing file paths on disk — the tool reads them automatically.`,
  ...
  send_developer_feedback: `Send freeform feedback to the Analyst Agent development team.
