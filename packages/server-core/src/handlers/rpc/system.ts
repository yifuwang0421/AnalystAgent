...
function parseInternalAnalystAgentDeepLink(parsed: URL): ParsedInternalDeepLink | null {
  if (parsed.protocol !== 'analystagent:') return null
...
export function registerSystemCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  // Shell operations - open URL in external browser (or handle analystagent:// internally)
  server.handle(RPC_CHANNELS.shell.OPEN_URL, async (ctx, url: string) => {
    ...
    try {
      ...
      if (parsed.protocol === 'analystagent:') {
        const deepLink = parseInternalAnalystAgentDeepLink(parsed)
        ...
        if (deepLink?.navigation?.view || deepLink?.navigation?.action) {
          ...
          deps.platform.logger.info('[OPEN_URL] Routing analystagent:// URL internally via deeplink:navigate')
        ...
        deps.platform.logger.info('[OPEN_URL] Falling back to client openExternal for analystagent:// URL')
