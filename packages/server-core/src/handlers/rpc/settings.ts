...
export function registerSettingsHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.workspace.SETTINGS_GET, async (_ctx, workspaceId: string) => {
    ...
    const { loadWorkspaceConfig, ensureFinanceWorkspaceConfig, saveWorkspaceConfig } = await import('@craft-agent/shared/workspaces')
    const config = loadWorkspaceConfig(workspace.rootPath)
    if (config && ensureFinanceWorkspaceConfig(workspace.rootPath, config)) {
  ...
  server.handle(RPC_CHANNELS.workspace.SETTINGS_UPDATE, async (_ctx, workspaceId: string, key: string, value: unknown) => {
    ...
    const validKeys = ['name', 'model', 'enabledSourceSlugs', 'permissionMode', 'cyclablePermissionModes', 'thinkingLevel', 'workingDirectory', 'localMcpEnabled', 'defaultLlmConnection', 'finance']
    ...
    if (key === 'name') {
      ...
      const { normalizeFinanceWorkspaceConfig, ensureFinanceResearchWorkspace } = await import('@craft-agent/shared/workspaces')
      config.finance = normalizeFinanceWorkspaceConfig(workspace.rootPath, normalizedValue as FinanceWorkspaceConfig)
      const validation = isValidWorkingDirectory(config.finance.researchDirectory)
      if (!validation.valid) {
        throw new Error(validation.reason!)
      }
      ensureFinanceResearchWorkspace(config.finance.researchDirectory)
    } else {
