...
import { isAbsolute, join, resolve, dirname, parse as parsePath, relative, extname, normalize } from 'path'
import { homedir } from 'os'
import { validatePathFormat } from '../../utils/path-validation'
import { randomUUID } from 'crypto'
import { RPC_CHANNELS, type FileAttachment, type DirectoryListingResult, type WorkspaceKnowledgeSearchResponse, type WorkspaceKnowledgeSearchResult, type WorkspaceResearchFileEntry, type WorkspaceResearchFilesResult, type WriteWorkspaceMarkdownResult } from '@craft-agent/shared/protocol'
...
const KNOWLEDGE_TEXT_EXTENSIONS = new Set(['.md', '.markdown', '.txt', '.csv', '.json', '.html', '.htm'])
const KNOWLEDGE_INDEXABLE_EXTENSIONS = new Set([
  '.md',
  '.markdown',
  '.txt',
  '.csv',
...
const SKIP_RESEARCH_DIRS = new Set([
  'node_modules', '.git', '.svn', '.hg', 'dist', 'build',
  '.next', '.nuxt', '.cache', '__pycache__', 'vendor',
  '.idea', '.vscode', 'coverage', '.nyc_output', '.turbo', 'out',
])
const MAX_RESEARCH_FILE_ENTRIES = 1000
const MAX_RESEARCH_TREE_DEPTH = 8
const MAX_KNOWLEDGE_SEARCH_FILES = 300
const MAX_KNOWLEDGE_TEXT_FILE_BYTES = 1_000_000
...
function getWorkspaceResearchContext(workspaceId?: string | null): { rootPath: string; knowledgeBaseEnabled: boolean } {
  if (!workspaceId) {
  ...
  const configuredRoot = config?.finance?.researchDirectory || workspace.rootPath
  const researchRoot = resolve(configuredRoot)
  if (!isInsidePath(workspace.rootPath, researchRoot)) {
    throw new Error('Configured research directory is outside the workspace')
  }

  return {
    rootPath: researchRoot,
    knowledgeBaseEnabled: config?.finance?.knowledgeBaseEnabled !== false,
  }
}

function getWorkspaceResearchRoot(workspaceId?: string | null): string {
  return getWorkspaceResearchContext(workspaceId).rootPath
}

function ensureAllowedResearchMarkdownPath(filePath: string, researchRoot: string): string {
  if (!filePath || typeof filePath !== 'string' || !isAbsolute(filePath)) {
    throw new Error('Only absolute Markdown file paths are allowed')
  }

  const resolvedPath = resolve(filePath)
  if (!isInsidePath(researchRoot, resolvedPath)) {
    throw new Error('Access denied: file path is outside the research directory')
  }

  const relativePath = toPosixRelativePath(relative(researchRoot, resolvedPath))
  const firstSegment = relativePath.split('/')[0]
  if (!FINANCE_RESEARCH_DIRS.includes(firstSegment as typeof FINANCE_RESEARCH_DIRS[number])) {
    throw new Error('Access denied: Markdown writes are limited to finance research folders')
  }

  if (!isMarkdownPath(resolvedPath)) {
    throw new Error('Only .md and .markdown files can be edited in the research workspace')
  }

  return resolvedPath
}

async function readResearchDirectory(
  rootPath: string,
  relativeDir: string,
  counter: { count: number },
  depth = 0,
): Promise<WorkspaceResearchFileEntry[]> {
  if (counter.count >= MAX_RESEARCH_FILE_ENTRIES || depth > MAX_RESEARCH_TREE_DEPTH) return []
  ...
  for (const entry of entries) {
    if (counter.count >= MAX_RESEARCH_FILE_ENTRIES) break
    if (entry.name.startsWith('.') || SKIP_RESEARCH_DIRS.has(entry.name)) continue

    const childRelative = toPosixRelativePath(relativeDir ? `${relativeDir}/${entry.name}` : entry.name)
    const absolutePath = join(rootPath, childRelative)
    ...
    if (entry.isDirectory()) {
      ...
      result.push({
        ...
        type: 'directory',
        children: await readResearchDirectory(rootPath, childRelative, counter, depth + 1),
      })
      continue
    }
...
function scoreKnowledgeText(text: string, terms: string[]): number {
  const lower = text.toLowerCase()
  return terms.reduce((score, term) => score + lower.split(term).length - 1, 0)
}

function extractKnowledgeTitle(filePath: string, content?: string): string {
  if (content) {
    const heading = content.split(/\r?\n/).find(line => /^#{1,3}\s+\S/.test(line))
    if (heading) return heading.replace(/^#{1,3}\s+/, '').trim()
  }
  return parsePath(filePath).base
}

function makeKnowledgeSnippet(content: string, terms: string[]): string {
...
async function collectKnowledgeSearchFiles(rootPath: string): Promise<string[]> {
  const knowledgeRoot = join(rootPath, 'knowledge')
  if (!isInsidePath(rootPath, knowledgeRoot)) {
    throw new Error('Access denied: knowledge path is outside the research directory')
  }

  const files: string[] = []
  const visit = async (dirPath: string, depth = 0): Promise<void> => {
    if (files.length >= MAX_KNOWLEDGE_SEARCH_FILES || depth > MAX_RESEARCH_TREE_DEPTH) return
    let entries: import('fs').Dirent[]
    try {
      entries = await readdir(dirPath, { withFileTypes: true })
    } catch {
    ...
    for (const entry of entries) {
      if (files.length >= MAX_KNOWLEDGE_SEARCH_FILES) break
      if (entry.name.startsWith('.') || SKIP_RESEARCH_DIRS.has(entry.name)) continue
      ...
      if (entry.isFile() && KNOWLEDGE_INDEXABLE_EXTENSIONS.has(extname(entry.name).toLowerCase())) {
        files.push(fullPath)
      }
    }
  }
...
export function registerFilesHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.file.LIST_WORKSPACE_RESEARCH_FILES, async (ctx, workspaceId?: string): Promise<WorkspaceResearchFilesResult> => {
    try {
      const resolvedWorkspaceId = workspaceId ?? ctx.workspaceId ?? deps.windowManager?.getWorkspaceForWindow(ctx.webContentsId!)
      const rootPath = getWorkspaceResearchRoot(resolvedWorkspaceId)
      const counter = { count: 0 }
      const entries = await Promise.all(FINANCE_RESEARCH_DIRS.map(async (dirName) => {
        const relativePath = dirName
        const absolutePath = join(rootPath, dirName)
        const children = await readResearchDirectory(rootPath, relativePath, counter)
        return {
          name: dirName,
          path: absolutePath,
          relativePath,
      ...
      const message = error instanceof Error ? error.message : 'Unknown error'
      deps.platform.logger.error('listWorkspaceResearchFiles error:', message)
      throw new Error(`Failed to list research files: ${message}`)
    }
  })

  server.handle(RPC_CHANNELS.file.SEARCH_WORKSPACE_KNOWLEDGE, async (ctx, workspaceId?: string, query = '', maxResults = 8): Promise<WorkspaceKnowledgeSearchResponse> => {
    const trimmedQuery = String(query ?? '').trim()
    try {
      const resolvedWorkspaceId = workspaceId ?? ctx.workspaceId ?? deps.windowManager?.getWorkspaceForWindow(ctx.webContentsId!)
      const { rootPath, knowledgeBaseEnabled } = getWorkspaceResearchContext(resolvedWorkspaceId)
      ...
      const terms = tokenizeKnowledgeQuery(trimmedQuery)
      if (terms.length === 0) {
        return {
          query: trimmedQuery,
          resultCount: 0,
          results: [],
          note: 'Enter a search query to search the workspace knowledge base.',
        }
      }

      const limit = Math.max(1, Math.min(Number.isFinite(maxResults) ? Math.floor(maxResults) : 8, 20))
      const hits: Array<WorkspaceKnowledgeSearchResult & { score: number }> = []

      for (const filePath of await collectKnowledgeSearchFiles(rootPath)) {
        if (!isInsidePath(rootPath, filePath)) continue
        const info = await stat(filePath).catch(() => null)
        if (!info?.isFile()) continue

        const ext = extname(filePath).toLowerCase()
        const relativePath = toPosixRelativePath(relative(rootPath, filePath))
        const filenameScore = scoreKnowledgeText(relativePath, terms)

        if (KNOWLEDGE_TEXT_EXTENSIONS.has(ext) && info.size <= MAX_KNOWLEDGE_TEXT_FILE_BYTES) {
          const content = await readFile(filePath, 'utf-8')
          const contentScore = scoreKnowledgeText(content, terms)
          const score = contentScore * 3 + filenameScore
          if (score > 0) {
            hits.push({
              path: filePath,
              relativePath,
              title: extractKnowledgeTitle(filePath, content),
              snippet: makeKnowledgeSnippet(content, terms),
        ...
        if (filenameScore > 0) {
          hits.push({
            path: filePath,
            relativePath,
            title: parsePath(filePath).base,
            snippet: `Binary or large file matched by filename. Use the system app to inspect it. Relative path: ${relativePath}`,
            updatedAt: info.mtime.toISOString(),
            sourceType: 'filename',
            score: filenameScore,
          })
      ...
      const results = hits.slice(0, limit).map(({ score: _score, ...hit }) => hit)
      return {
        query: trimmedQuery,
      ...
      const message = error instanceof Error ? error.message : 'Unknown error'
      deps.platform.logger.error('searchWorkspaceKnowledge error:', message)
      throw new Error(`Failed to search workspace knowledge: ${message}`)
    }
  })

  server.handle(RPC_CHANNELS.file.WRITE_WORKSPACE_MARKDOWN, async (ctx, path: string, content: string, expectedMtimeMs?: number): Promise<WriteWorkspaceMarkdownResult> => {
    try {
      const workspaceId = ctx.workspaceId ?? deps.windowManager?.getWorkspaceForWindow(ctx.webContentsId!)
      const researchRoot = getWorkspaceResearchRoot(workspaceId)
      const safePath = ensureAllowedResearchMarkdownPath(path, researchRoot)
      const info = await stat(safePath)
      const realTargetPath = await realpath(safePath)

      if (!info.isFile()) {
        throw new Error('Markdown target is not a file')
      }
      if (!isInsidePath(researchRoot, realTargetPath)) {
        throw new Error('Access denied: resolved Markdown path is outside the research directory')
      }
      if (typeof expectedMtimeMs === 'number' && Math.abs(info.mtimeMs - expectedMtimeMs) > 1) {
        throw new Error('Stale write rejected: file has changed on disk')
      }

      await writeFile(safePath, content, 'utf-8')
      const updated = await stat(safePath)
      return { ok: true, mtimeMs: updated.mtimeMs }
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unknown error'
      deps.platform.logger.error('writeWorkspaceMarkdown error:', message)
      throw new Error(`Failed to write Markdown file: ${message}`)
    }
  })
