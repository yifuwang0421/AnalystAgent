...
DOWNLOAD_DIR="$HOME/.analyst-agent/downloads"
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    platform="linux-${arch}"
    APP_NAME="Analyst-Agent-x64.AppImage"
...
if [ -z "$filename" ]; then
    filename="Analyst-Agent-${arch}.${ext}"
fi
...
if [ "$OS_TYPE" = "darwin" ]; then
    ...
    APP_BUNDLE_ID="com.analystagent.desktop"
    if pgrep -x "Analyst Agent" >/dev/null 2>&1; then
        ...
        if pgrep -x "Analyst Agent" >/dev/null 2>&1; then
            ...
            pkill -9 -x "Analyst Agent" 2>/dev/null || true
    ...
    echo ""
    printf "%b\n" "  Analyst Agent has been installed to ${BOLD}$INSTALL_DIR/$APP_NAME${NC}"
    echo ""
    printf "%b\n" "  You can launch it from ${BOLD}Applications${NC} or by running:"
    printf "%b\n" "    ${BOLD}open -a 'Analyst Agent'${NC}"
    echo ""
...
if [ ! -f "$APPIMAGE_PATH" ]; then
    ...
    echo "Reinstall: curl -fsSL https://github.com/yifuwang0421/AnalystAgent/install-app.sh | bash"
    exit 1
fi
...
WRAPPER_EOF
    ...
    OLD_APPIMAGE="$INSTALL_DIR/Analyst-Agent-x64.AppImage"
