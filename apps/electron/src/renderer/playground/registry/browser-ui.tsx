...
function getLiveFxPayload(scenario: Scenario, runState: RunState): { active: boolean; label: string; cursor: { x: number; y: number } | null } {
  ...
  if (runState === 'running') {
    ...
    return {
      active: true,
      label: 'Analyst Agent are working…',
...
function BrowserAgentEmptyState({
  ...
  const handlePromptSelect = useCallback(async (prompt: string) => {
    ...
    const deepLinkUrl = `analystagent://${deepLinkRoute}`
...
const MOCK_BROWSER_PRESETS: Record<BrowserTabStripMockPreset, BrowserInstanceInfo[]> = {
  ...
  'long-names': [
    {
      id: 'long-1',
      url: 'https://www.notion.so/Analyst-Agent-Multi-Session-Browser-Registry-Design-Review-Thread-2026-Q1',
      title: 'Analyst Agent Multi-Session Browser Registry Design Review Thread (Q1 2026)',
      favicon: null,
      isLoading: false,
      canGoBack: true,
      canGoForward: true,
  ...
  'stress-mix': [
    ...
    },
    {
      id: 'mix-4',
      url: 'https://github.com/lukilabs/craft-agents-oss',
      title: 'Analyst Agent OSS Repo with a Surprisingly Long Branch and Compare View Name',
      favicon: null,
      isLoading: false,
      canGoBack: true,
      canGoForward: true,
