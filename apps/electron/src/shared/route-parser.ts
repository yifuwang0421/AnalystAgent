...
export interface ParsedCompoundRoute {
  ...
  /** File workspace scope (only for files navigator) */
  filesScope?: 'research' | 'knowledge'
...
export function parseCompoundRoute(route: string): ParsedCompoundRoute | null {
  ...
  // Files / knowledge navigator.
  if (first === 'files' || first === 'knowledge') {
    const scope = first === 'knowledge'
      ? 'knowledge'
      : (segments[1] === 'knowledge' ? 'knowledge' : 'research')
    const detailIndex = first === 'knowledge' ? 1 : (segments[1] === 'research' || segments[1] === 'knowledge' ? 2 : 1)
    if (segments[detailIndex] === 'file' && segments[detailIndex + 1]) {
      return {
        navigator: 'files',
        filesScope: scope,
        details: { type: 'file', id: decodeURIComponent(segments[detailIndex + 1]) },
      }
    }
    return { navigator: 'files', filesScope: scope, details: null }
  }
...
function convertCompoundToViewRoute(compound: ParsedCompoundRoute): ParsedRoute {
  ...
  if (compound.navigator === 'files') {
    if (!compound.details) {
      return { type: 'view', name: compound.filesScope === 'knowledge' ? 'knowledge' : 'files', params: {} }
    }
    return { type: 'view', name: 'file-workspace', id: compound.details.id, params: { scope: compound.filesScope ?? 'research' } }
  }
  ...
  if (compound.navigator === 'agents') {
    ...
    return { type: 'view', name: 'agent-preset', id: compound.details.id, params: {} }
  }
...
function convertParsedRouteToNavigationState(parsed: ParsedRoute): NavigationState | null {
  ...
  switch (parsed.name) {
    ...
    case 'files':
      return { navigator: 'files', scope: 'research', details: null }
    case 'knowledge':
      return { navigator: 'files', scope: 'knowledge', details: null }
    case 'file-workspace':
      return {
        navigator: 'files',
        scope: parsed.params.scope === 'knowledge' ? 'knowledge' : 'research',
