...
export const routes = {
  ...
  action: {
     ...
     * @param agentId - Optional investment research agent preset ID to attach to the new session
     */
  ...
  view: {
    ...
    /** Research file workspace (center work area). */
    files: (params?: { scope?: 'research' | 'knowledge'; path?: string }) => {
      const scope = params?.scope ?? 'research'
      const base = scope === 'knowledge' ? 'knowledge' : 'files/research'
      if (params?.path) {
        return `${base}/file/${encodeURIComponent(params.path)}` as const
      }
      return base as 'knowledge' | 'files/research'
    },

    /** Knowledge-base file workspace shortcut. */
    knowledge: (path?: string) => {
      if (path) return `knowledge/file/${encodeURIComponent(path)}` as const
      return 'knowledge' as const
    },

    /** Fixed analyst role presets. */
    agents: (agentId?: string) => {
      if (agentId) return `agents/agent/${agentId}` as const
