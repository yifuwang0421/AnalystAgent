 ...
 * Deep Link Handler
 *
 * Parses analystagent:// URLs and routes to appropriate actions.
 *
 * URL Formats (workspace is optional - uses active window if omitted):
 *
 * Compound format (hierarchical navigation):
 *   analystagent://allSessions[/session/{sessionId}]            - Session list (all sessions)
 *   analystagent://flagged[/session/{sessionId}]             - Session list (flagged filter)
 *   analystagent://state/{stateId}[/session/{sessionId}]     - Session list (state filter)
 *   analystagent://sources[/source/{sourceSlug}]          - Sources list
 *   analystagent://settings[/{subpage}]                   - Settings (general, shortcuts, preferences)
 *
 * Action format:
 *   analystagent://action/{actionName}[/{id}][?params]
 *   analystagent://workspace/{workspaceId}/action/{actionName}[?params]
 *
 * Actions:
 ...
 *   analystagent://allSessions/session/abc123                (specific session)
 *   analystagent://settings/shortcuts                     (shortcuts page)
 *   analystagent://sources/source/github                  (github source info)
 *   analystagent://action/new-chat                        (uses active window)
 *   analystagent://action/resume-sdk-session/{sdkId}      (resume Claude Code session)
 *   analystagent://workspace/ws123/allSessions/session/abc123   (targets specific workspace)
 */
...
import { ANALYST_DEEPLINK_SCHEME } from './branding'
...
export function parseDeepLink(url: string): DeepLinkTarget | null {
  try {
    ...
    if (parsed.protocol !== `${ANALYST_DEEPLINK_SCHEME}:`) {
      return null
    }

    // For custom protocols, the hostname contains the first path segment
    // e.g., analystagent://workspace/ws123 鈫?hostname='workspace', pathname='/ws123'
    // e.g., analystagent://allSessions/chat/abc 鈫?hostname='allSessions', pathname='/chat/abc'
    const host = parsed.hostname
    const pathParts = parsed.pathname.split('/').filter(Boolean)
    const windowMode = parseWindowMode(parsed)
    const rightSidebar = parseRightSidebar(parsed)

    // analystagent://auth-callback?... (OAuth callbacks - return null to let existing handler process)
    ...
    // analystagent://allSessions/..., analystagent://settings/..., etc. (compound routes)
    ...
    // analystagent://workspace/{workspaceId}/... (with workspace targeting)
    if (host === 'workspace') {
    ...
    // analystagent://action/... (no workspace - uses active window)
    if (host === 'action') {
