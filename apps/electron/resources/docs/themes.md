...
This guide explains how to customize the visual theme of Analyst Agent.

## Overview

Analyst Agent uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.analyst-agent/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.analyst-agent/theme.json` - Override specific colors (app-level)
...
```
~/.analyst-agent/workspaces/{id}/config.json
```
...
Create `~/.analyst-agent/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.analyst-agent/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.analyst-agent/themes/{name}.json`
...
1. Create `~/.analyst-agent/theme.json` for overrides or `~/.analyst-agent/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.analyst-agent/theme.json` for overrides, `~/.analyst-agent/themes/` for presets)
