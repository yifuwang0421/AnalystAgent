...
This guide explains how to create and configure skills in Analyst Agent.
...
Analyst Agent uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in Analyst Agent
...
1. **Workspace skill checked first** - If `~/.analyst-agent/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.analyst-agent/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.analyst-agent/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.analyst-agent/workspaces/{ws}/skills/commit/SKILL.md`
