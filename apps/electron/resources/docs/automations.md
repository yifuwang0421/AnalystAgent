...
This guide explains how to configure automations in Analyst Agent to automate workflows based on events.
...
Automations allow you to trigger actions automatically when specific events occur in Analyst Agent. You can:
...
```
~/.analyst-agent/workspaces/{workspaceId}/automations.json
```
...
### App Events (triggered by Analyst Agent)
...
Send a prompt to Analyst Agent (creates a new session for scheduled prompts).
...
```json
{
  ...
  "method": "POST",
  "body": { "text": "Hello from Analyst Agent!" }
}
```
...
4. **Pair the supergroup.** In Analyst Agent: Settings → Messaging → Telegram → **Pair Supergroup**. Copy the 6-digit code, then in any topic of the supergroup type `/pair <code>`. The bot confirms and the Settings row updates with the group's title.

Verify by checking the supergroup row in Settings shows the group title. If automation runs fail later, `~/.analyst-agent/logs/messaging-gateway.log` will show `automation_topic_bind_failed` with the underlying Telegram error.
...
Ask Analyst Agent to validate your automations configuration:
...
2. **Check env vars** — Ensure `CRAFT_WH_*` variables are set in your shell profile and Analyst Agent was restarted after adding them. URLs using `$VAR` templates are validated after variable expansion — if the variable is empty or unset, the URL will be invalid.
