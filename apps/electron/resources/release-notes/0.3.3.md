...
When your OAuth token expires, Analyst Agent automatically refreshes it in the background using the refresh token. No manual re-authentication required — your MCP connections stay alive.
...
Configure sources that require multiple authentication headers (e.g., API key + tenant ID). See the updated documentation at `~/.analyst-agent/docs/sources.md`.
...
- Lazy loading of bundled docs fixes packaged app initialization (upstream #162)
