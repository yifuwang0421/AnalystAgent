...
### Why it's cool
This is the first step toward full programmatic control of Analyst Agent. Script your agent workflows, integrate into CI/CD pipelines, or run sessions headlessly on remote machines. Partially addresses #343.

## Headless Server

Run Analyst Agent as a standalone server without the Electron desktop app — deploy on any machine, connect from any client.
...
### Why it's cool
Deploy Analyst Agent on a remote server and connect from anywhere. Run it on your home lab, a cloud VM, or a CI runner. The thin-client mode means the browser UI stays lightweight while all the heavy lifting happens server-side. Partially addresses #291.
