#!/usr/bin/env bun
/**
 * craft-cli — Terminal client for Analyst Agent server.
 *
 * Connects over WebSocket (ws:// or wss://) to a running Analyst Agent server
  ...
  return [
    ...
    // ----- MCP source validation (pre-committed in .github/agents/sources/) -----
    {
      ...
      fn: async (client, ctx) => {
        ...
        return await waitForSendEvents(client, ctx.createdSessionId,
          `[source:craft-public] List the documents under the "AnalystAgent E2E Test" folder inside the "AnalystAgent" folder. Just list their names.`,
...
function printHelp(): void {
  process.stdout.write(`craft-cli — Terminal client for Analyst Agent server
